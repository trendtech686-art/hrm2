...
const Select = ({ value, defaultValue, ...props }: SelectProps) => {
  ...
  if (defaultValue !== undefined) controlledProps.defaultValue = defaultValue;
  return <SelectPrimitive.Root {...controlledProps} {...props} />;
};
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-base ring-offset-background data-placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:h-9 md:text-sm [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
