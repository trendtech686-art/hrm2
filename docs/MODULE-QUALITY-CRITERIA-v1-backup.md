...
> 🚫 **TUYỆT ĐỐI KHÔNG HARDCODE LIMIT (BẮT BUỘC)**
>
> Dữ liệu sẽ tăng dần theo thời gian (1-2-3 năm). Bất kỳ `limit` cố định nào
> (ví dụ `limit: 100`, `limit: 500`, `limit: 1000`) đều sẽ **CẮT MẤT DỮ LIỆU** khi vượt ngưỡng.
>
> | Tình huống | ❌ Sai | ✅ Đúng |
> |-----------|--------|--------|
> | **Trang danh sách** (table) | `useTasks({ limit: 1000 })` | `useTasks({ page, limit: pageSize })` — pagination từ UI |
> | **Dropdown/Select** (cần tất cả) | `useAllEmployees({ limit: 500 })` | Auto-pagination: fetch page 1 → discover `totalPages` → fetch remaining pages song song |
> | **Detail page** (1 entity) | Load toàn bộ rồi `.filter()` | Server-side filter: `usePenaltiesByEmployee(employeeId)` — API trả đúng 1 employee |
> | **Thống kê/Metrics** | Load raw data rồi tính client-side | Server-side aggregation: API endpoint trả kết quả đã tính (count, sum, avg) |
> | **Search/Autocomplete** | Hardcode `limit = 20` | Function parameter `limit = 20` — caller có thể override, data đã lọc sẵn |
>
> **Nguyên tắc:**
> - `limit` trong pagination UI (user chọn pageSize) → ✅ Cho phép
> - `limit` là function parameter (caller truyền vào) → ✅ Cho phép
> - `limit` hardcode trong hook/API client → 🚫 CẤM
> - Constant `PAGE_SIZE` cho auto-pagination → ✅ Cho phép (đây là batch size, không phải cap)
>
> **Pattern Auto-Pagination (cho hook cần TẤT CẢ data):**
> ```typescript
> const PAGE_SIZE = 100; // Batch size, KHÔNG phải limit cap
>
> export function useAllEmployees() {
>   // Step 1: Fetch page 1 → discover totalPages
>   const firstPage = useQuery({
>     queryKey: keys.list({ page: 1, limit: PAGE_SIZE }),
>     queryFn: () => fetchData({ page: 1, limit: PAGE_SIZE }),
>   });
...
>     queries: Array.from({ length: totalPages - 1 }, (_, i) => ({
>       queryKey: keys.list({ page: i + 2, limit: PAGE_SIZE }),
>       queryFn: () => fetchData({ page: i + 2, limit: PAGE_SIZE }),
>       enabled: totalPages > 1 && !!firstPage.data,
>     })),
>   });
>
>   // Step 3: Merge all pages
>   return [...firstPage.data, ...remaining.flatMap(q => q.data)];
> }
> ```
...
>
> Khi hiển thị dữ liệu liên quan đến **1 entity cụ thể** (ví dụ: chi tiết nhân viên),
> **PHẢI** dùng server-side filter — KHÔNG ĐƯỢC load toàn bộ rồi `.filter()` client-side.
>
> | Tình huống | ❌ Sai (Anti-pattern) | ✅ Đúng |
> |-----------|----------------------|--------|
> | Penalties của 1 NV | `usePenalties()` → `.filter(p => p.employeeId === id)` | `usePenaltiesByEmployee(employeeId)` → API `?employeeSystemId=X` |
> | Leaves của 1 NV | `useLeaves()` → `.filter(l => l.employeeId === id)` | `useLeavesByEmployee(employeeId)` → API `?employeeId=X` |
> | Tasks của 1 NV | `useTasks()` → `.filter(t => t.assigneeId === id)` | `useTasksByEmployee(employeeId)` → API `?assigneeId=X` |
> | Payslips của 1 NV | `useAllPayslips()` → `.filter()` | `usePayslipsByEmployee(employeeId)` → API `?employeeId=X` |
>
> **Lý do:**
> - Load toàn bộ 10,000 records chỉ để lấy 5 records của 1 NV = **lãng phí bandwidth + memory**
> - Server đã có index trên `employeeId` → query nhanh O(log n)
> - Client-side filter = O(n) với n ngày càng tăng theo thời gian
>
> **Hook pattern đúng cho `ByEmployee`:**
> ```typescript
> // ✅ Server-side filtered — API trả đúng data của 1 employee
> export function usePenaltiesByEmployee(employeeSystemId: string | undefined) {
>   const query = usePenalties({ employeeSystemId }); // param gửi lên server
