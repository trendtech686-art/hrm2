...
const ROUTE_METADATA: Record<string, { label: string; icon?: string }> = {
  ...
  '/purchase-orders': { label: 'Đơn nhập hàng' },
  '/purchase-returns': { label: 'Trả hàng nhập' },
  '/ordered-products': { label: 'Hàng đặt' },
  '/cashbook': { label: 'Sổ quỹ' },
  '/complaints': { label: 'Khiếu nại' },
  '/cost-adjustments': { label: 'Điều chỉnh chi phí' },
  '/inventory': { label: 'Kho hàng' },
  '/inventory-checks': { label: 'Kiểm kho' },
  '/inventory-receipts': { label: 'Phiếu nhập kho' },
  '/my-tasks': { label: 'Công việc của tôi' },
  '/notifications': { label: 'Thông báo' },
  '/organization-chart': { label: 'Sơ đồ tổ chức' },
  '/packaging': { label: 'Đóng gói' },
  '/payroll': { label: 'Bảng lương' },
  '/price-adjustments': { label: 'Điều chỉnh giá' },
  '/profile': { label: 'Hồ sơ cá nhân' },
  '/reconciliation': { label: 'Đối soát COD' },
  '/reports': { label: 'Báo cáo' },
  '/returns': { label: 'Trả hàng' },
  '/sales-returns': { label: 'Trả hàng bán' },
  '/shipments': { label: 'Vận đơn' },
  '/stock-locations': { label: 'Vị trí kho' },
  '/stock-transfers': { label: 'Chuyển kho' },
  '/supplier-warranties': { label: 'Bảo hành NCC' },
