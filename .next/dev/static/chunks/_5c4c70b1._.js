...
"[project]/lib/import-export/configs/payment.config.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const getPaymentTypeStore = ()=>__TURBOPACK__imported__module__$5b$project$5d2f$features$2f$settings$2f$payments$2f$types$2f$store$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["usePaymentTypeStore"].getState();
const getPaymentMethodStore = ()=>__TURBOPACK__imported__module__$5b$project$5d2f$features$2f$settings$2f$payments$2f$methods$2f$store$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["usePaymentMethodStore"].getState();
const getTargetGroupStore = ()=>__TURBOPACK__imported__module__$5b$project$5d2f$features$2f$settings$2f$target$2d$groups$2f$store$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useTargetGroupStore"].getState();
...
const findPaymentType = (identifier)=>{
    if (!identifier) return undefined;
    const store = getPaymentTypeStore();
    const normalized = identifier.trim().toLowerCase();
    return store.data.find((t)=>t.id.toLowerCase() === normalized || t.name.toLowerCase().includes(normalized));
};
const findPaymentMethod = (identifier)=>{
    if (!identifier) return undefined;
    const store = getPaymentMethodStore();
    const normalized = identifier.trim().toLowerCase();
    return store.data.find((m)=>m.id.toLowerCase() === normalized || m.name.toLowerCase().includes(normalized));
};
const getDefaultPaymentMethod = ()=>{
    const store = getPaymentMethodStore();
    return store.data.find((m)=>m.isDefault) || store.data[0];
};
const findTargetGroup = (identifier)=>{
    if (!identifier) return undefined;
    const store = getTargetGroupStore();
    const normalized = identifier.trim().toLowerCase();
    return store.data.find((g)=>g.id.toLowerCase() === normalized || g.name.toLowerCase().includes(normalized));
};
const findRecipient = (recipientType, recipientName)=>{
    ...
    if (recipientType.includes('Khách hàng') || recipientType === 'KHACHHANG') {
        ...
        return store.data.find((c)=>c.id.toLowerCase() === normalized || c.name.toLowerCase().includes(normalized));
    }
    if (recipientType.includes('Nhà cung cấp') || recipientType === 'NHACUNGCAP') {
        const store = getSupplierStore();
        return store.data.find((s)=>s.id.toLowerCase() === normalized || s.name.toLowerCase().includes(normalized));
    }
    if (recipientType.includes('Nhân viên') || recipientType === 'NHANVIEN') {
        const store = getEmployeeStore();
        return store.data.find((e)=>e.id.toLowerCase() === normalized || e.fullName?.toLowerCase().includes(normalized));
    }
    return undefined;
};
const paymentFields = [
...
const paymentImportExportConfig = {
    entityType: 'payments',
    ...
    validateRow: (row, _index, existingData, mode)=>{
        ...
        if (!payment.paymentReceiptTypeName) {
            errors.push({
                field: 'paymentReceiptTypeName',
                message: 'Loại phiếu chi không được để trống'
            });
        }
        // Check duplicate
        if (mode === 'insert-only' && payment.id) {
            const duplicate = existingData.find((p)=>p.id.toString().toUpperCase() === payment.id.toString().toUpperCase());
            if (duplicate) {
                errors.push({
                    field: 'id',
                    message: `Mã phiếu "${payment.id}" đã tồn tại`
                });
            }
        }
        return errors;
    ...
    postTransformRow: (row)=>{
        ...
        // Lookup entities
        const branch = findBranch(row.branchName) || defaultBranch;
        const paymentType = findPaymentType(row.paymentReceiptTypeName);
        const paymentMethod = findPaymentMethod(row.paymentMethodName) || defaultPaymentMethod;
        const targetGroup = findTargetGroup(row.recipientTypeName);
        const recipient = findRecipient(row.recipientTypeName || '', row.recipientName || '');
        return {
            systemId: (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$id$2d$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["asSystemId"])(''),
            id: (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$id$2d$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["asBusinessId"])(String(row.id || '').trim()),
            date: String(row.date || now.split('T')[0]),
            amount: Number(row.amount) || 0,
            recipientTypeSystemId: targetGroup?.systemId || (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$id$2d$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["asSystemId"])('KHAC'),
            ...
            paymentMethodSystemId: paymentMethod?.systemId || (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$id$2d$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["asSystemId"])(''),
            ...
            paymentReceiptTypeSystemId: paymentType?.systemId || (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$id$2d$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["asSystemId"])(''),
            paymentReceiptTypeName: paymentType?.name || String(row.paymentReceiptTypeName || ''),
            branchSystemId: branch?.systemId || (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$id$2d$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["asSystemId"])(''),
            branchName: branch?.name || '',
            createdBy: (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$id$2d$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["asSystemId"])(''),
            createdAt: now,
...
"[project]/features/payments/components/payment-import-export-dialogs.tsx [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
;
;
function PaymentImportDialog({ open, onOpenChange, branches, existingData, onImport, currentUser }) {
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(__TURBOPACK__imported__module__$5b$project$5d2f$components$2f$shared$2f$generic$2d$import$2d$dialog$2d$v2$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__["GenericImportDialogV2"], {
        ...
        fileName: "[project]/features/payments/components/payment-import-export-dialogs.tsx",
        lineNumber: 57,
        columnNumber: 9
    }, this);
}
_c = PaymentImportDialog;
function PaymentExportDialog({ open, onOpenChange, allData, filteredData, currentPageData, selectedData, currentUser }) {
...
"[project]/features/payments/components/payment-import-export-dialogs.tsx [app-client] (ecmascript, next/dynamic entry)", ((__turbopack_context__) => {

__turbopack_context__.n(__turbopack_context__.i("[project]/features/payments/components/payment-import-export-dialogs.tsx [app-client] (ecmascript)"));
}),
]);

//# sourceMappingURL=_5c4c70b1._.js.map