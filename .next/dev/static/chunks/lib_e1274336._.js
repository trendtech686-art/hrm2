...
"[project]/lib/print/stock-in-print-helper.ts [app-client] (ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
;
;
function convertStockInForPrint(stockIn, options = {}) {
    ...
    return {
        ...
        items: stockIn.items.map((item)=>{
            const qty = item.receivedQuantity ?? item.quantity ?? item.orderedQuantity ?? 0;
            const discountAmount = item.discountType === 'percentage' ? item.unitPrice * Number(qty) * (item.discount || 0) / 100 : (item.discount || 0) * Number(qty);
...
//# sourceMappingURL=lib_e1274336._.js.map