...
export const POST = apiHandler(async (request, { session }) => {
    ...
    const result = await prisma.customer.updateMany({
      where: {
        ...
        isDeleted: true,
        permanentlyDeletedAt: null, // Không thể khôi phục đã lưu trữ vĩnh viễn
      },
      data: {
        isDeleted: false,
        deletedAt: null,
    ...
    getUserNameFromDb(session!.user.id).then(userName =>
      prisma.activityLog.create({
        data: {
          ...
          action: 'restored',
          actionType: 'update',
          note: `Khôi phục hàng loạt ${result.count} khách hàng: ${customers.map(c => c.name).join(', ')}`,
