...
export async function GET(
  ...
  try {
    ...
    logError('[Customer Types API] GET by ID error', error);
...
export async function PATCH(
  ...
  try {
    ...
    if (!existing || existing.isDeleted) return apiNotFound('CustomerType');
    ...
    if (Object.keys(changes).length > 0) {
      ...
      createActivityLog({
        ...
        entityId: systemId,
        action: `Cập nhật loại khách hàng: ${existing.name}: ${changeDetail}`,
        actionType: 'update',
        changes,
    ...
    logError('[Customer Types API] PATCH error', error);
  ...
  try {
    ...
    if (existing) {
      createActivityLog({
        entityType: 'customer_settings',
        entityId: systemId,
        action: `Xóa loại khách hàng: ${existing.name}`,
        actionType: 'delete',
        createdBy: session.user?.id,
      }).catch(e => logError('Failed to create activity log', e))
    }
    ...
    logError('[Customer Types API] DELETE error', error);
