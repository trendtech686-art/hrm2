...
export async function GET() {
  ...
  try {
    ...
    logError('[Customer Types API] GET error', error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const setting = await prisma.customerSetting.create({
      data: {
        id: await generateIdWithPrefix('CTYPE', prisma),
        name,
        type: TYPE,
        description,
        color,
    ...
    createActivityLog({
      ...
      action: `Thêm loại khách hàng: ${name}`,
      actionType: 'create',
    ...
    logError('[Customer Types API] POST error', error);
