...
export async function GET(
  ...
  try {
    ...
    logError('[Payment Terms API] GET by ID error', error);
...
export async function PATCH(
  ...
  try {
    ...
    if (!existing || existing.isDeleted) return apiNotFound('PaymentTerm');
    ...
    if (Object.keys(changes).length > 0) {
      ...
      createActivityLog({
        ...
        entityId: systemId,
        action: `Cập nhật điều khoản thanh toán: ${existing.name}: ${changeDetail}`,
        actionType: 'update',
        changes,
    ...
    logError('[Payment Terms API] PATCH error', error);
  ...
  try {
    ...
    if (existing) {
      createActivityLog({
        entityType: 'customer_payment_term',
        entityId: systemId,
        action: `Xóa điều khoản thanh toán: ${existing.name}`,
        actionType: 'delete',
        createdBy: session.user?.id,
      }).catch(e => logError('Failed to create activity log', e))
    }
    ...
    logError('[Payment Terms API] DELETE error', error);
