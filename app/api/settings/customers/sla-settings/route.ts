...
// POST - Create a new SLA setting
export async function POST(request: NextRequest) {
  const session = await requireAuth();
  if (!session) return apiError('Unauthorized', 401);

  const validation = await validateBody(request, createSlaSettingSchema);
  ...
  try {
    ...
    const setting = await prisma.customerSetting.create({
      data: {
        id: await generateIdWithPrefix('SLA', prisma),
        name,
        type: TYPE,
        description,
        color,
