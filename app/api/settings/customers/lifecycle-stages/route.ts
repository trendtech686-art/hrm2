...
export async function GET() {
  try {
    ...
    console.error('[Lifecycle Stages API] GET error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[Lifecycle Stages API] POST error:', error);
    return NextResponse.json({ error: 'Failed to create lifecycle stage' }, { status: 500 });
  }
}
