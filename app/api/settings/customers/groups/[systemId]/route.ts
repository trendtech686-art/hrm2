...
export async function GET(
  ...
  try {
    ...
    logError('[Customer Groups API] GET by ID error', error);
...
export async function PATCH(
  ...
  try {
    ...
    if (!existing || existing.isDeleted) return apiNotFound('Customer group');
    ...
    if (Object.keys(changes).length > 0) {
      ...
      createActivityLog({
        ...
        entityId: systemId,
        action: `Cập nhật nhóm khách hàng: ${existing.name}: ${changeDetail}`,
        actionType: 'update',
        changes,
    ...
    logError('[Customer Groups API] PATCH error', error);
  ...
  try {
    ...
    if (existing) {
      createActivityLog({
        entityType: 'customer_group',
        entityId: systemId,
        action: `Xóa nhóm khách hàng: ${existing.name}`,
        actionType: 'delete',
        createdBy: session.user?.id,
      }).catch(e => logError('Failed to create activity log', e))
    }
    ...
    logError('[Customer Groups API] DELETE error', error);
