...
export async function GET(_req: Request, { params }: { params: Promise<{ systemId: string }> }) {
  ...
  try {
    ...
    if (!item) return apiError('Payment type not found', 404)
    return apiSuccess(mapRecord(item as unknown as SettingsDataRecord))
  } catch (error) {
    logError('[payment-types] GET detail error', error)
    return apiError('Failed to fetch payment type', 500)
  }
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ systemId: string }> }) {
  ...
  try {
    ...
    if (!existing || existing.type !== TYPE) return apiError('Payment type not found', 404)
    ...
    if (Object.keys(changes).length > 0) {
      ...
      createActivityLog({
        ...
        entityId: systemId,
        action: `Cập nhật loại thanh toán: ${existing.name}: ${changeDetail}`,
        actionType: 'update',
        changes,
    ...
    logError('[payment-types] PATCH error', error)
    return apiError('Failed to update payment type', 500)
  }
}
  ...
  try {
    ...
    if (existing) {
      createActivityLog({
        entityType: 'payment_type',
        entityId: systemId,
        action: `Xóa loại thanh toán: ${existing.name}`,
        actionType: 'delete',
        createdBy: session.user?.id,
      }).catch(e => logError('Failed to create activity log', e))
    }
    ...
    if (prismaError?.code === 'P2025') return apiError('Payment type not found', 404)
    logError('[payment-types] DELETE error', error)
    return apiError('Failed to delete payment type', 500)
  }
}
