...
export async function POST(_req: Request, { params }: { params: Promise<{ systemId: string }> }) {
  ...
  try {
    ...
    if (!existing) return apiError('Target group not found', 404)
    ...
    createActivityLog({
      ...
      entityId: systemId,
      action: `Cập nhật nhóm đối tượng: ${existing.name}: Mặc định`,
      actionType: 'update',
    ...
    logError('[target-groups] set-default error', error)
    return apiError('Failed to set default target group', 500)
  }
}
