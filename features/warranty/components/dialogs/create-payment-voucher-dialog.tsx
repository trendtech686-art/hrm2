...
interface CreatePaymentVoucherDialogProps {
  ...
  branchName?: string;
  existingPayments?: any[]; // Phiếu chi đã tạo
}
...
export function CreatePaymentVoucherDialog({
  ...
  const currentUser = React.useMemo(() => 
    employees.find(e => e.systemId === currentUserSystemId),
    [employees, currentUserSystemId]
  );
  ...
  const onSubmit = (values: FormValues) => {
    try {
      ...
      const payment: Omit<Payment, 'systemId'> = {
        id: '' as any, // Let store generate PC-XXXXXX ID
        date: toISODateTime(now) || now.toISOString(),
        amount: values.amount,
        
        // Recipient info (TargetGroup)
        recipientTypeSystemId: '', // TODO: Get KHACHHANG systemId from TargetGroup
        ...
        linkedOrderSystemId: linkedOrderSystemId, // Link đến đơn hàng (nếu trừ vào đơn)
