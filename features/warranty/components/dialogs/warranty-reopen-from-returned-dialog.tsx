...
export function WarrantyReopenFromReturnedDialog({ open, onOpenChange, ticket }: WarrantyReopenFromReturnedDialogProps) {
  ...
  const handleReopen = React.useCallback(() => {
    ...
    try {
      ...
      const targetStatus = ticket.status === 'RETURNED' ? 'COMPLETED' : 'PROCESSING';
      
      // ⚠️ KHÔNG ĐỘNG CHẠM GÌ KHI MỞ LẠI TỪ RETURNED
      // Lý do:
      // - Trả hàng = Đơn đã xong, hàng đã xuất, tiền đã thanh toán
      ...
      if (ticket.status === 'RETURNED') {
        // No-op: RETURNED tickets are reopened for viewing only
      }
      
      // ✅ Pass lý do mở lại vào note parameter
      update.mutate({ systemId: ticket.systemId, data: { status: targetStatus } });
      
      // Clear returnedAt và linkedOrderSystemId only when going back to COMPLETED (not from RETURNED)
