...
import { calculateWarrantyProcessingState } from '../warranty-processing-logic.ts';

interface CreateReceiptVoucherDialogProps {
  warrantyId: string;
  ...
  branchName?: string;
  existingReceipts?: any[]; // Phiếu thu đã tạo
}
...
export function CreateReceiptVoucherDialog({
  warrantyId,
  warrantySystemId,
  customer,
  defaultAmount = 0,
  ...
  branchName,
  existingReceipts = [],
}: CreateReceiptVoucherDialogProps) {
  const [open, setOpen] = React.useState(false);
  const navigate = useNavigate();
  
  const { add: addReceipt, data: receipts } = useReceiptStore();
  ...
  const ticket = React.useMemo(() => findById(warrantySystemId), [findById, warrantySystemId]);

  const { remainingAmountForReceipt, totalPaymentForTicket } = React.useMemo(() => {
    if (!ticket) {
      return { remainingAmountForReceipt: 0, totalPaymentForTicket: 0 };
    }
    ...
    const state = calculateWarrantyProcessingState(ticket, payments, receipts, totalPayment);
    return {
      remainingAmountForReceipt: state.remainingAmount,
      totalPaymentForTicket: totalPayment,
    };
  ...
  const onSubmit = (values: FormValues) => {
    try {
      const now = new Date();
      const latestTicket = findById(warrantySystemId);
      if (!latestTicket) {
      ...
      const totalPaymentFromTicket = latestTicket.products.reduce((sum, p) => {
        if (p.resolution === 'out_of_stock') {
          return sum + ((p.quantity || 0) * (p.unitPrice || 0));
        }
        return sum;
      }, 0) + (latestTicket.shippingFee || 0);
      ...
      const receipt: Omit<Receipt, 'systemId'> = {
        ...
        payerTypeSystemId: '', // TODO: Get KHACHHANG systemId from TargetGroup
        ...
        paymentMethodSystemId: '', // TODO: Get payment method systemId
        paymentMethodName: values.paymentMethod,
        
        // Account & Type - TODO: Get from settings
        accountSystemId: '', // TODO: Get default cash account
        paymentReceiptTypeSystemId: '', // TODO: Get "Thu thêm bảo hành" type
        ...
        // Optional fields
        completedAt: toISODateTime(now) || now.toISOString(),
        
        // Links to warranty
        linkedWarrantySystemId: warrantySystemId, // Link đến phiếu bảo hành
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              {amount > 0 ? `${amount.toLocaleString('vi-VN')} đ` : 'Chưa nhập'}
            </p>
          </div>
