...
export function StockTransfersPage({ initialStats }: StockTransfersPageProps = {}) {
  ...
  usePageHeader({ title: 'Danh sách chuyển kho', breadcrumb: [{ label: 'Trang chủ', href: ROUTES.ROOT, isCurrent: false }, { label: 'Chuyển kho', href: '/stock-transfers', isCurrent: true }], actions: React.useMemo(() => [canCreate && <Button key="add" onClick={() => router.push(ROUTES.INVENTORY.STOCK_TRANSFER_NEW)}><Plus className="mr-2 h-4 w-4" />Tạo phiếu chuyển kho</Button>].filter(Boolean), [router, canCreate]), showBackButton: false });
  ...
  return (
    <ListPageShell>
      ...
      <PageFilters searchValue={searchQuery} onSearchChange={setSearchQuery} searchPlaceholder="Tìm kiếm phiếu chuyển kho...">
        <Select value={fromBranchFilter} onValueChange={setFromBranchFilter}><SelectTrigger className="w-full sm:w-45"><SelectValue placeholder="Chi nhánh chuyển" /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả CN chuyển</SelectItem>{branches.map(b => <SelectItem key={b.systemId} value={b.systemId}>{b.name}</SelectItem>)}</SelectContent></Select>
        <Select value={toBranchFilter} onValueChange={setToBranchFilter}><SelectTrigger className="w-full sm:w-45"><SelectValue placeholder="Chi nhánh nhận" /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả CN nhận</SelectItem>{branches.map(b => <SelectItem key={b.systemId} value={b.systemId}>{b.name}</SelectItem>)}</SelectContent></Select>
