...
import { Plus, Printer, FileSpreadsheet, Download, Truck, CheckCircle2, XCircle, Clock, Settings } from 'lucide-react';
...
export function StockTransfersPage({ initialStats }: StockTransfersPageProps = {}) {
  ...
  return (
    <div className="flex flex-col w-full h-full">
      {!isMobile && <PageToolbar leftActions={<><Button variant="outline" size="sm" onClick={() => router.push('/settings/inventory')}><Settings className="h-4 w-4 mr-2" />Cài đặt</Button><Button variant="outline" size="sm" onClick={() => setShowImportDialog(true)}><FileSpreadsheet className="mr-2 h-4 w-4" />Nhập file</Button><Button variant="outline" size="sm" onClick={() => setShowExportDialog(true)}><Download className="mr-2 h-4 w-4" />Xuất Excel</Button></>} rightActions={<DataTableColumnCustomizer columns={columns} columnVisibility={columnVisibility} setColumnVisibility={setColumnVisibility} columnOrder={columnOrder} setColumnOrder={setColumnOrder} pinnedColumns={pinnedColumns} setPinnedColumns={setPinnedColumns} onResetToDefault={resetColumnLayout} />} />}
      ...
      <StatsCardGrid columns={4} className="my-4">
        <StatsCard title="Chờ chuyển" value={stats?.pending ?? 0} icon={Clock} variant="warning" />
        <StatsCard title="Đang chuyển" value={stats?.inTransit ?? 0} icon={Truck} variant="info" />
        <StatsCard title="Hoàn thành" value={stats?.completed ?? 0} icon={CheckCircle2} variant="success" />
        <StatsCard title="Đã hủy" value={stats?.cancelled ?? 0} icon={XCircle} variant="danger" />
      </StatsCardGrid>
