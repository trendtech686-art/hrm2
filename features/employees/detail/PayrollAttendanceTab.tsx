...
export function PayrollAttendanceTab({ employee }: PayrollAttendanceTabProps) {
  ...
  const payrollColumns: ColumnDef<PayrollHistoryRow>[] = React.useMemo(() => [
    ...
    { id: 'otherDeductions', accessorKey: 'otherDeductions', header: () => <span className="text-right w-full block">Khác</span>, size: 90, cell: ({ row }) => <span className="text-right block tabular-nums text-muted-foreground">{row.otherDeductions > 0 ? `-${formatCurrency(row.otherDeductions)}` : '—'}</span>, meta: { displayName: 'Khấu trừ khác' } },
