...
export function EmployeesPage({ initialStats }: EmployeesPageProps = {}) {
  ...
  const headerActions = React.useMemo(() => [...(canDelete ? [<Button key="trash" variant="outline" size="sm" onClick={() => router.push('/employees/trash')}><Trash2 className="mr-2 h-4 w-4" />Thùng rác ({deletedCount})</Button>] : []), ...(canCreate ? [<Button key="add" size="sm" onClick={() => router.push('/employees/new')}><PlusCircle className="mr-2 h-4 w-4" />Thêm nhân viên</Button>] : [])], [router, deletedCount, canCreate, canDelete]);
