...
export function InventoryChecksPage({ initialStats }: InventoryChecksPageProps) {
  ...
  const headerActions = React.useMemo(() => [canCreate && <Button key="add" onClick={() => router.push(ROUTES.INVENTORY.INVENTORY_CHECK_NEW)}><Plus className="mr-2 h-4 w-4" />Tạo phiếu kiểm hàng</Button>].filter(Boolean), [router, canCreate]);
