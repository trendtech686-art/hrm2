...
import { Plus, Download, Printer, XCircle, Scale, FileSpreadsheet, FileEdit, Loader, CheckCircle2, Settings } from 'lucide-react';
...
export function InventoryChecksPage({ initialStats }: InventoryChecksPageProps) {
  ...
  const columns = React.useMemo(() => getColumns(handleEdit, requestCancel, requestBalance, router, handlePrint, findEmployeeById), [handleEdit, router, requestCancel, requestBalance, handlePrint, findEmployeeById]);
  ...
  return (
    <div className="space-y-4">
      ...
      {!isMobile && <PageToolbar leftActions={<><Button variant="outline" size="sm" onClick={() => router.push('/settings/inventory')}><Settings className="h-4 w-4 mr-2" />Cài đặt</Button><Button variant="outline" size="sm" onClick={() => setShowImportDialog(true)}><FileSpreadsheet className="mr-2 h-4 w-4" />Nhập file</Button><Button variant="outline" size="sm" onClick={() => setShowExportDialog(true)}><Download className="mr-2 h-4 w-4" />Xuất Excel</Button></>} rightActions={columns?.length ? <DataTableColumnCustomizer columns={columns} columnVisibility={columnVisibility} setColumnVisibility={setColumnVisibility} columnOrder={columnOrder} setColumnOrder={setColumnOrder} pinnedColumns={pinnedColumns} setPinnedColumns={setPinnedColumns} /> : null} />}
      ...
      {isMobile && mobileLoadedCount < data.length && <div className="py-6 text-center"><div className="flex items-center justify-center gap-2 text-muted-foreground"><div className="h-4 w-4 animate-spin rounded-full border-2 border-primary border-t-transparent" /><span className="text-body-sm">Đang tải thêm...</span></div></div>}
      {isMobile && mobileLoadedCount >= data.length && data.length > 20 && <div className="py-6 text-center"><p className="text-body-sm text-muted-foreground">Đã hiển thị tất cả {data.length} kết quả</p></div>}
