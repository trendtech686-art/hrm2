...
export function SlaSettingsTabContent({ isActive, onRegisterActions }: TabContentProps) {
  ...
  const handleSave = React.useCallback(() => {
    setIsSaving(true);
    try { update(localSettingsRef.current); toast.success('Đã lưu cài đặt cảnh báo tồn kho'); }
    catch (_error) { toast.error('Có lỗi xảy ra khi lưu cài đặt'); }
    finally { setIsSaving(false); }
  }, [update]);

  React.useEffect(() => { if (!isActive) return; onRegisterActions([<SettingsActionButton key="save-sla" onClick={handleSave} disabled={!hasChanges || isSaving}><Save className="mr-2 h-4 w-4" />{isSaving ? 'Đang lưu...' : 'Lưu cài đặt'}</SettingsActionButton>]); }, [isActive, hasChanges, isSaving, handleSave, onRegisterActions]);
