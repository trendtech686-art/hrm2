...
// Importers
export async function fetchImporters(): Promise<Importer[]> {
  const res = await fetch(`${BASE_URL}/importers`);
  if (!res.ok) throw new Error('Failed to fetch');
  return res.json();
}

export async function createImporter(data: Record<string, unknown>): Promise<Importer> {
  const res = await fetch(`${BASE_URL}/importers`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
  if (!res.ok) throw new Error('Failed to create');
  return res.json();
}

export async function updateImporter(systemId: string, data: Record<string, unknown>): Promise<Importer> {
