...
export function BrandsPage() {
  ...
  const { data: brandsData, isLoading: isLoadingBrands, isFetching } = useBrands({
  ...
  const totalRows = brandsData?.pagination?.total ?? 0;
  const pageCount = brandsData?.pagination?.totalPages ?? 1;
  ...
  const handleImport = React.useCallback(async (importData: Partial<Brand>[], mode: 'insert-only' | 'update-only' | 'upsert') => {
    ...
    for (let i = 0; i < importData.length; i++) {
      ...
      try {
        const existing = brands.find(b => item.id && b.id === item.id);
        if (existing) {
  ...
  }, [columnVisibility, setColumnVisibility, setColumnOrder]);
  ...
  const allSelectedRows = React.useMemo(() => brands.filter(b => rowSelection[b.systemId]), [brands, rowSelection]);
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      ...
      <PageFilters searchValue={searchQuery} onSearchChange={setSearchQuery} searchPlaceholder="Tìm theo mã, tên...">
        <AdvancedFilterPanel
      ...
      <BrandImportDialog open={isImportOpen} onOpenChange={setIsImportOpen} existingData={brands as unknown as Brand[]} onImport={handleImport} currentUser={authEmployee ? { systemId: authEmployee.systemId, name: authEmployee.fullName || authEmployee.id } : undefined} />
      <BrandExportDialog open={isExportOpen} onOpenChange={setIsExportOpen} allData={brands as unknown as Brand[]} filteredData={brands as unknown as Brand[]} currentPageData={brands as unknown as Brand[]} selectedData={allSelectedRows as unknown as Brand[]} currentUser={authEmployee ? { systemId: authEmployee.systemId, name: authEmployee.fullName || authEmployee.id } : { systemId: asSystemId('SYSTEM'), name: 'System' }} />
    </div>
  );
}
