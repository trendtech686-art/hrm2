...
/**
 * DeliverySourceReport Page
 *
 * Báo cáo giao hàng theo nguồn đơn hàng
 */
...
import { useDeliverySourceReport } from '../hooks/use-delivery-report';
import { ResponsiveDataTable } from '@/components/data-table/responsive-data-table';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import type { ColumnDef } from '@/components/data-table/types';
import type { ReportDateRange, DeliverySourceReportRow, ChartType } from '../types';
import type { SystemId } from '@/lib/id-types';
import { Globe, Truck, TrendingUp, CheckCircle, Filter } from 'lucide-react';
...
const getColumns = (): ColumnDef<DeliverySourceReportRow & { systemId: string; _isSummary?: boolean }>[] => [
  {
    id: 'sourceName',
...
export function DeliverySourceReportPage() {
  ...
  const { data, isLoading, isError, error } = useDeliverySourceReport(dateRange);

  const tableData = React.useMemo(() => {
    const summaryRow: DeliverySourceReportRow & { systemId: SystemId; _isSummary: boolean } = {
      sourceName: 'Tổng',
      ...
      sourceId: '__summary__' as unknown as SystemId,
  ...
  const summaryCards = React.useMemo(() => [
    { title: 'Số bản ghi', value: data.length, icon: Globe },
  ...
  const headerActions = React.useMemo(() => (
    <ReportHeaderActions
      title="Báo cáo giao hàng theo nguồn đơn hàng"
  ...
  usePageHeader({
    title: 'Báo cáo giao hàng theo nguồn đơn hàng',
    subtitle: 'Tình hình giao hàng theo từng nguồn',
    breadcrumb: [
      { label: 'Trang chủ', href: ROUTES.ROOT },
      { label: 'Báo cáo', href: ROUTES.REPORTS.INDEX },
      { label: 'Giao hàng theo nguồn', href: ROUTES.REPORTS.DELIVERY_BY_SOURCE, isCurrent: true },
    ],
    showBackButton: true,
  ...
  const renderMobileCard = (row: DeliverySourceReportRow & { systemId: string; _isSummary?: boolean }) => (
    <div className={`p-4 space-y-3 ${row._isSummary ? 'bg-muted/50' : ''}`}>
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        ...
        <div><span className="text-muted-foreground">Tỷ lệ giao:</span> {row.deliveryRate.toFixed(1) + '%'}</div>
      </div>
    </div>
  );
...
export default DeliverySourceReportPage;
