...
import type { ReportDateRange, SalesCustomerGroupReportRow, ChartType } from '../types';
import type { SystemId } from '@/lib/id-types';
import { Users, DollarSign, TrendingUp, ShoppingCart, Filter } from 'lucide-react';

const getColumns = (): ColumnDef<SalesCustomerGroupReportRow & { systemId: string; _isSummary?: boolean }>[] => [
  {
    id: 'groupName',
    ...
    cell: ({ row }) => (
      <div className={row._isSummary ? 'font-semibold' : ''}>
        {row._isSummary ? 'Tổng' : row.groupName}
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    size: 100,
    enableSorting: true,
    cell: ({ row }) => <span className="text-right block">{formatCurrency(row.averageOrderValue)}</span>,
  }
];
...
export function SalesCustomerGroupReportPage() {
  ...
  const tableData = React.useMemo(() => {
    const summaryRow: SalesCustomerGroupReportRow & { systemId: SystemId; _isSummary: boolean } = {
      groupId: '__summary__',
      groupName: 'Tổng',
      customerCount: data.reduce((s, r) => s + r.customerCount, 0),
    ...
    return [summaryRow, ...data.map(row => ({
      ...row,
      systemId: (row.groupId as unknown as SystemId),
      _isSummary: false,
    }))];
  }, [data, summary]);
  ...
  const exportColumns = React.useMemo(() => [
    { key: 'groupName', label: 'Nhóm khách hàng', selected: true },
    { key: 'customerCount', label: 'Số khách hàng', selected: true },
    { key: 'orderCount', label: 'SL đơn hàng', selected: true },
    { key: 'productAmount', label: 'Tiền hàng', selected: true },
    { key: 'returnAmount', label: 'Trả lại', selected: true },
    { key: 'revenue', label: 'Doanh thu', selected: true },
    { key: 'grossProfit', label: 'Lợi nhuận gộp', selected: true },
    { key: 'averageOrderValue', label: 'TB/đơn', selected: true }
  ], []);

  const summaryCards = React.useMemo(() => [
    { title: 'Số nhóm KH', value: data.length, icon: Users },
  ...
  usePageHeader({
    title: 'Báo cáo bán hàng theo nhóm khách hàng',
    subtitle: 'Phân tích doanh thu theo nhóm khách hàng',
    breadcrumb: [
      { label: 'Trang chủ', href: ROUTES.ROOT },
      { label: 'Báo cáo', href: ROUTES.REPORTS.INDEX },
      { label: 'Bán hàng theo nhóm KH', href: ROUTES.REPORTS.SALES_BY_CUSTOMER_GROUP, isCurrent: true },
    ],
    showBackButton: true,
  ...
  const renderMobileCard = (row: SalesCustomerGroupReportRow & { systemId: string; _isSummary?: boolean }) => (
    <div className={`p-4 space-y-3 ${row._isSummary ? 'bg-muted/50' : ''}`}>
      <div className="flex items-center justify-between">
        <span className={`font-medium ${row._isSummary ? 'text-base' : ''}`}>
          {row._isSummary ? 'Tổng cộng' : row.groupName}
        </span>
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle>Chi tiết theo nhóm khách hàng</CardTitle>
...
export default SalesCustomerGroupReportPage;
