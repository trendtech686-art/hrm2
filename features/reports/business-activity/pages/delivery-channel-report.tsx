...
import type { ReportDateRange, DeliveryChannelReportRow, ChartType } from '../types';
import type { SystemId } from '@/lib/id-types';
import { Radio, Truck, TrendingUp, CheckCircle, Filter } from 'lucide-react';
...
const getColumns = (): ColumnDef<DeliveryChannelReportRow & { systemId: string; _isSummary?: boolean }>[] => [
  {
    id: 'channelName',
    ...
    cell: ({ row }) => (
      <div className={row._isSummary ? 'font-semibold' : ''}>
        {row._isSummary ? 'Tổng' : row.channelName}
      </div>
    ),
  },
  {
...
export function DeliveryChannelReportPage() {
  ...
  const tableData = React.useMemo(() => {
    const summaryRow: DeliveryChannelReportRow & { systemId: SystemId; _isSummary: boolean } = {
      ...
      channelId: '__summary__' as unknown as SystemId,
    ...
    return [summaryRow, ...data.map(row => ({
      ...row,
      systemId: (row.channelId as unknown as SystemId),
      _isSummary: false,
    }))];
  }, [data]);
  ...
  const exportColumns = React.useMemo(() => [
    { key: 'channelName', label: 'Kênh bán hàng', selected: true },
  ...
  const summaryCards = React.useMemo(() => [
    { title: 'Số bản ghi', value: data.length, icon: Radio },
  ...
  usePageHeader({
    title: 'Báo cáo giao hàng theo kênh bán hàng',
    subtitle: 'Tình hình giao hàng theo từng kênh',
    breadcrumb: [
      { label: 'Trang chủ', href: ROUTES.ROOT },
      { label: 'Báo cáo', href: ROUTES.REPORTS.INDEX },
      { label: 'Giao hàng theo kênh', href: ROUTES.REPORTS.DELIVERY_BY_CHANNEL, isCurrent: true },
    ],
    showBackButton: true,
  ...
  const renderMobileCard = (row: DeliveryChannelReportRow & { systemId: string; _isSummary?: boolean }) => (
    <div className={`p-4 space-y-3 ${row._isSummary ? 'bg-muted/50' : ''}`}>
      <div className="flex items-center justify-between">
        <span className={`font-medium ${row._isSummary ? 'text-base' : ''}`}>
          {row._isSummary ? 'Tổng cộng' : row.channelName}
        </span>
...
export default DeliveryChannelReportPage;
