...
import { useDeliveryEmployeeReport } from '../hooks/use-delivery-report';
import { ResponsiveDataTable } from '@/components/data-table/responsive-data-table';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import type { ColumnDef } from '@/components/data-table/types';
import type { ReportDateRange, DeliveryEmployeeReportRow, ChartType } from '../types';
import type { SystemId } from '@/lib/id-types';
import { UserCheck, Truck, TrendingUp, CheckCircle, Filter } from 'lucide-react';
...
const getColumns = (): ColumnDef<DeliveryEmployeeReportRow & { systemId: string; _isSummary?: boolean }>[] => [
  {
    id: 'employeeName',
    ...
    cell: ({ row }) => (
      <div className={row._isSummary ? 'font-semibold' : ''}>
        {row._isSummary ? 'Tổng' : row.employeeName}
      </div>
    ),
  },
  {
...
export function DeliveryEmployeeReportPage() {
  ...
  const { data, isLoading, isError, error } = useDeliveryEmployeeReport(dateRange);

  const tableData = React.useMemo(() => {
    const summaryRow: DeliveryEmployeeReportRow & { systemId: SystemId; _isSummary: boolean } = {
      employeeName: 'Tổng',
      ...
      employeeSystemId: '__summary__' as unknown as SystemId,
  ...
  const summaryCards = React.useMemo(() => [
    { title: 'Số bản ghi', value: data.length, icon: UserCheck },
  ...
  usePageHeader({
    title: 'Báo cáo giao hàng theo nhân viên',
    subtitle: 'Tình hình giao hàng theo từng nhân viên',
    breadcrumb: [
      { label: 'Trang chủ', href: ROUTES.ROOT },
      { label: 'Báo cáo', href: ROUTES.REPORTS.INDEX },
      { label: 'Giao hàng theo NV', href: ROUTES.REPORTS.DELIVERY_BY_EMPLOYEE, isCurrent: true },
    ],
    showBackButton: true,
  ...
  const renderMobileCard = (row: DeliveryEmployeeReportRow & { systemId: string; _isSummary?: boolean }) => (
...
export default DeliveryEmployeeReportPage;
