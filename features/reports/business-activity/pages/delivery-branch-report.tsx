...
import { useDeliveryBranchReport } from '../hooks/use-delivery-report';
import { ResponsiveDataTable } from '@/components/data-table/responsive-data-table';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import type { ColumnDef } from '@/components/data-table/types';
import type { ReportDateRange, DeliveryBranchReportRow, ChartType } from '../types';
import type { SystemId } from '@/lib/id-types';
import { Building2, Truck, TrendingUp, CheckCircle, Filter } from 'lucide-react';
...
const getColumns = (): ColumnDef<DeliveryBranchReportRow & { systemId: string; _isSummary?: boolean }>[] => [
  {
    id: 'branchName',
...
export function DeliveryBranchReportPage() {
  ...
  const { data, isLoading, isError, error } = useDeliveryBranchReport(dateRange);

  const tableData = React.useMemo(() => {
    const summaryRow: DeliveryBranchReportRow & { systemId: SystemId; _isSummary: boolean } = {
      branchName: 'Tổng',
      ...
      branchSystemId: '__summary__' as unknown as SystemId,
  ...
  const summaryCards = React.useMemo(() => [
    { title: 'Số bản ghi', value: data.length, icon: Building2 },
  ...
  usePageHeader({
    title: 'Báo cáo giao hàng theo chi nhánh',
    subtitle: 'Tình hình giao hàng theo từng chi nhánh',
    breadcrumb: [
      { label: 'Trang chủ', href: ROUTES.ROOT },
      { label: 'Báo cáo', href: ROUTES.REPORTS.INDEX },
      { label: 'Giao hàng theo CN', href: ROUTES.REPORTS.DELIVERY_BY_BRANCH, isCurrent: true },
    ],
    showBackButton: true,
  ...
  const renderMobileCard = (row: DeliveryBranchReportRow & { systemId: string; _isSummary?: boolean }) => (
...
export default DeliveryBranchReportPage;
