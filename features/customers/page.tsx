...
export function CustomersPage({ initialStats, initialGroups: _initialGroups }: CustomersPageProps = {}) {
  ...
  const headerActions = React.useMemo(() => [
    canDelete && <Button key="trash" variant="outline" size="sm" className="h-9" asChild><Link href="/customers/trash"><Trash2 className="mr-2 h-4 w-4" />Thùng rác ({deletedCount})</Link></Button>,
    canCreate && <Button key="add" size="sm" className="h-9" asChild><Link href="/customers/new"><PlusCircle className="mr-2 h-4 w-4" />Thêm khách hàng</Link></Button>,
          ...
          {isMobile ? (
            ...
            <>
              ...
              <PageFilters searchValue={tableState.search} onSearchChange={handleSearchChange} searchPlaceholder="Tìm kiếm khách hàng...">
                ...
                <Select value={tableState.debtFilter} onValueChange={handleDebtFilterChange}><SelectTrigger className="w-48 h-9"><SelectValue placeholder="Công nợ" /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả công nợ</SelectItem><SelectItem value="customerOwes">Phải thu (khách nợ mình)</SelectItem><SelectItem value="weOwe">Phải trả (mình nợ khách)</SelectItem><SelectItem value="hasDebt">Có công nợ</SelectItem><SelectItem value="noDebt">Không có công nợ</SelectItem><SelectItem value="overdue">Quá hạn thanh toán</SelectItem><SelectItem value="dueSoon">Sắp đến hạn</SelectItem></SelectContent></Select>
